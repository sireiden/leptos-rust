...
This starter was extended to stream live data over a WebSocket at `/ws` using Axum. The server broadcasts a timestamp every second using a Tokio broadcast channel; the frontend connects after hydration and renders the latest ticks.

- Server route: `GET /ws` (Upgrades to WebSocket)
- Client connects to `ws(s)://<current-host>/ws` automatically

You can test it by running the dev server below and loading the home page; you should see a "Live Ticks" list updating once per second without a full page refresh.
...
```text
LEPTOS_OUTPUT_NAME="rust-leptos-sandbox"
